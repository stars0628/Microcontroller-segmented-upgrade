...
#error 根据需求修改两个define，完成后注释本行
#define FLASH_APP1_ADDR (0x08000000 + 20 * 1024) /*给BOOT预留20kb空间，APP20kb之后存储*/
#define OTHER_LENGTH    2

#error 请在初始化结构体时给other_message_length赋值，然后注释本代码
...
// #error 请将串口长度获取到结构体中长度值后，然后注释本代码
...
void Transmission_and_Upgrade(uint8_t *serial_data, uint32_t length);
